...
def get_all_records_cached():
    ...
    with engine.connect() as conn:
        result = conn.execute(
            text(
                """
            SELECT AGE_GROUP_ARAB, AGE_GROUP_ENGL, NATIONALITY_ARAB, NATIONALITY_ENGL,
                OBSVALUE_OBSV, SEX_ARAB, SEX_ENGL, YEAR_TIME
            FROM umrah_records
        """
            )
        )
        all_records_cache = [dict(row._mapping) for row in result]
...
async def get_filtered_statistics(
    ...
    dimensions: Optional[List[str]] = Query(
        None, description="Filter dimensions (AGE_GROUP, NATIONALITY)"
    ),
    # Individual field filters
    AGE_GROUP_ENGL: Optional[str] = Query(None, description="Filter by age group"),
    NATIONALITY_ENGL: Optional[str] = Query(
    ...
    orderby: Optional[str] = Query(
        None, description="Order results by field name, prefix with '-' for descending"
    ),
    ...
    # Format
    format: Optional[Literal["JSON", "HTML", "CSV"]] = Query(
        "JSON", description="Response format (JSON, HTML, CSV)"
    ),
):
    """
    ...
    try:
        ...
        if format_upper == "JSON":
            ...
            return Response(
                ...
                media_type="text/csv",
                headers={
                    "Content-Disposition": "attachment; filename=umrah_statistics.csv"
                },
            )
...
async def get_metadata():
    ...
    try:
        ...
        years = (
            db.query(UmrahRecord.YEAR_TIME)
            .distinct()
            .order_by(UmrahRecord.YEAR_TIME)
            .all()
        )
        ...
        return {
            ...
            "fields": {
                ...
                "REGION_CODE": {
                    "type": "string",
                    "values": sorted([r[0] for r in region_codes if r[0]])
                    if region_codes
                    else [],
                },
                "REGION_ENGL": {
                    "type": "string",
                    "values": sorted([r[0] for r in region_names if r[0]])
                    if region_names
                    else [],
                },
            },
